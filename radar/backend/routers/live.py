...
async def process_and_ingest_event(payload: dict) -> str:
...
async def ingest_live_alert(request: Request):
    ...
    for payload in items:
        last_event_id = await process_and_ingest_event(payload)

    return {"status": "accepted", "events_processed": len(items), "last_event_id": last_event_id}


@router.post("/simulate-nmap")
async def simulate_nmap(body: dict):
    target_ip = body.get("ip")
    if not target_ip:
        raise HTTPException(status_code=400, detail="Target IP 'ip' is required.")

    # Auto-activate monitoring so simulated scan alerts are captured & processed
    app_state.monitoring_active = True
    app_state.feed_state = "LIVE_FEED_ACTIVE"
    ...
    async def run_sim():
        ...
        for port in target_ports:
            ...
            await process_and_ingest_event(payload)
            await asyncio.sleep(0.15)

    asyncio.create_task(run_sim())
    return {"status": "started", "message": f"Nmap port scan simulation started for IP: {target_ip}"}


@router.post("/simulate-ssh-brute")
async def simulate_ssh_brute(body: dict):
    target_ip = body.get("ip")
    if not target_ip:
        raise HTTPException(status_code=400, detail="Target IP 'ip' is required.")

    # Auto-activate monitoring so simulated brute force alerts are captured & processed
    app_state.monitoring_active = True
    app_state.feed_state = "LIVE_FEED_ACTIVE"
    ...
    asyncio.create_task(run_sim())
    return {"status": "started", "message": f"SSH brute force simulation started for IP: {target_ip}"}
