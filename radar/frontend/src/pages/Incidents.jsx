...
// ─── Playbook Detail ──────────────────────────────────────────────────────────
function PlaybookDetail({ alert, playbook, loading, onGenerate, onBack, active, report, loadingReport, onGenerateReport }) {
  const { state } = useStore()
    ...
    <div className={`flex-1 flex flex-col h-full min-w-0 ${active ? 'flex' : 'hidden md:flex'}`}>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6">
        ...
        <div className="card p-4 md:p-5 mb-4">
          ...
          {/* Generate Playbook & Report Buttons */}
          <div className="mt-4 flex flex-col items-end gap-3 pt-4 border-t border-outline/10">
            <div className="flex flex-wrap gap-2 justify-end">
              {/* Playbook Button */}
              {playbook ? (
                <span className="chip-success py-1.5 px-3 rounded text-xs font-mono font-bold flex items-center gap-1.5">
                  <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
                  Playbook Ready ({playbook.provider})
                </span>
              ) : (
                <button
                  onClick={onGenerate}
                  disabled={loading || !(state.aiEnabled || !!sessionStorage.getItem('radar_session_ai_key'))}
                  className={`btn-primary flex items-center gap-2 text-xs py-1.5 px-3.5 ${!(state.aiEnabled || !!sessionStorage.getItem('radar_session_ai_key')) ? 'opacity-40 cursor-not-allowed' : ''}`}
              ...
              {/* Report Button */}
              {report ? (
                <span className="chip-success py-1.5 px-3 rounded text-xs font-mono font-bold flex items-center gap-1.5" style={{ backgroundColor: 'rgba(59, 158, 255, 0.15)', borderColor: 'rgba(59, 158, 255, 0.3)', color: '#3b9eff' }}>
                  📝 Report Ready ({report.provider})
                </span>
              ) : (
                <button
                  onClick={onGenerateReport}
                  disabled={loadingReport || !(state.aiEnabled || !!sessionStorage.getItem('radar_session_ai_key'))}
                  className={`btn-primary flex items-center gap-2 text-xs py-1.5 px-3.5 ${!(state.aiEnabled || !!sessionStorage.getItem('radar_session_ai_key')) ? 'opacity-40 cursor-not-allowed' : ''}`}
                  style={report ? {} : { backgroundColor: 'rgba(59, 158, 255, 0.1)', borderColor: 'rgba(59, 158, 255, 0.2)', color: '#3b9eff' }}
            ...
            {!(state.aiEnabled || !!sessionStorage.getItem('radar_session_ai_key')) && (
              <p className="text-xs text-on-surface-variant text-right" id="ai-disabled-warning">
                Add your Gemini or Claude API key in Settings to enable AI-generated playbooks & reports
              </p>
            )}
          </div>
        </div>
        ...
        {report && (
          <div className="card p-5 space-y-4 mb-4 border border-primary/20 bg-surface-low animate-fade-in">
            <h3 className="mono-label text-primary font-bold text-sm border-b border-primary/10 pb-2 flex items-center gap-1.5">
              <span>📝</span> DETAILED INCIDENT REPORT
            </h3>
            
            <div className="space-y-4 text-xs md:text-sm">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <p className="mono-label text-[10px] text-on-surface-variant font-bold">TIME OF ACTIVITY</p>
                  <p className="mono-data text-on-surface mt-0.5">{report.time_of_activity}</p>
                </div>
                <div>
                  <p className="mono-label text-[10px] text-on-surface-variant font-bold">DETECTED</p>
                  <p className="mono-data text-on-surface mt-0.5">{report.detected}</p>
                </div>
              </div>

              <div>
                <p className="mono-label text-[10px] text-on-surface-variant font-bold">LIST OF AFFECTED ENTITIES</p>
                <p className="text-on-surface mt-0.5 whitespace-pre-wrap">{report.affected_entities}</p>
              </div>

              <div>
                <p className="mono-label text-[10px] text-on-surface-variant font-bold">RECIPIENT: REASON FOR CLASSIFYING AS TRUE POSITIVE OR FALSE POSITIVE</p>
                <p className="text-on-surface mt-0.5 whitespace-pre-wrap">{report.classification_reason}</p>
              </div>

              <div>
                <p className="mono-label text-[10px] text-on-surface-variant font-bold">REASON FOR ESCALATING THE ALERT</p>
                <p className="text-on-surface mt-0.5 whitespace-pre-wrap">{report.escalation_reason}</p>
              </div>

              <div>
                <p className="mono-label text-[10px] text-on-surface-variant font-bold">RECOMMENDED REMEDIATION ACTIONS</p>
                <p className="text-on-surface mt-0.5 whitespace-pre-wrap">{report.remediation_actions}</p>
              </div>

              <div>
                <p className="mono-label text-[10px] text-on-surface-variant font-bold">LIST OF ATTACK INDICATORS</p>
                <p className="text-on-surface mt-0.5 whitespace-pre-wrap">{report.attack_indicators}</p>
              </div>
            </div>
          </div>
        )}
...
export default function Incidents() {
  ...
  useEffect(() => {
    if (alertId) {
      ...
      if (found) {
        ...
        // Try to load existing playbook & report
        api.playbook.get(alertId).then(setPlaybook).catch(() => {})
        api.playbook.getReport(alertId).then(setReport).catch(() => {})
      }
    }
  ...
  const handleSelect = (ev) => {
    ...
    api.playbook.getReport(ev.id).then(setReport).catch(() => {})
  }
  ...
  const handleGenerate = async () => {
    ...
    try {
      const sessionProvider = sessionStorage.getItem('radar_session_ai_provider')
      const sessionKey = sessionStorage.getItem('radar_session_ai_key')
      const pb = await api.playbook.generate(
        selectedAlert.id,
      ...
      alert("Add your Gemini or Claude API key in Settings to enable AI-generated playbooks")
    } finally {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      const sessionProvider = sessionStorage.getItem('radar_session_ai_provider')
      const sessionKey = sessionStorage.getItem('radar_session_ai_key')
      const rep = await api.playbook.generateReport(
        selectedAlert.id,
        sessionProvider || undefined,
        sessionKey || undefined
      )
      ...
      alert("Add your Gemini or Claude API key in Settings to enable AI-generated reports")
    } finally {
